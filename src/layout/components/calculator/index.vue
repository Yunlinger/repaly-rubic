...
export default {
  ...
  methods: {
    ...
    checkNum: function(event, item) {
      // 对应1-9键
      this.expression === '0' ? ((this.expression = ''), (this.expression += item)) : (this.expression += item) // 如果当前算式只有一个0,则删除这个0再将数字拼入算式
    },
    turnFloat: function() {
      // 对应%,将数字转化为它*0.01后的结果
    ...
    calculate: function() {
      ...
      try {
        ...
        this.result = this.$math.format(this.$math.evaluate(this.expression))
      } catch (e) {
        alert('您输入的算式有误')
      }
    }
