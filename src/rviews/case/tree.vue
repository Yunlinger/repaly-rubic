    ...
    onDblClick(e, id, node) {
      ...
      if (this.currentNode) {
        if (!this.currentNode.isParent) {
          if (this.editable) {
            this.$router.push(`/engineer/case/edit/${this.currentNode.data.id}`)
          } else {
            this.$confirm('确定启动案例 ' + this.currentNode.data.name + ' ?', '提示', {
              confirmButtonText: '启动',
              cancelButtonText: '取消',
              type: 'warning'
            }).then(() => {
              return api.dispatch(this.currentNode.data.id)
            }).then(() => {
              return api.start(this.currentNode.data.id)
            }).then(() => {
              this.$msg.success(this.currentNode.data.name, '启动案例成功')
            }).catch(err => {
