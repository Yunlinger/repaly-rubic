...
*  给elementUI的dialog上加上 v-dialogdragresize 指令就可以实现弹窗的全屏和拉伸了。
...
*/
// v-dialogdragresize: 弹窗拖拽+水平方向伸缩
Vue.directive('dialogdragresize', {
  bind(el, binding, vnode, oldVnode) {
    ...
    let nowMarginTop = 0
    // 获取弹框头部（这部分可双击全屏）
    const dialogHeaderEl = el.querySelector('.rubik-dialog__header')
    // 弹窗
    // const dragDom = el.querySelector('.rubik-dialog')
    const dragDom = el
    ...
    const moveDown = (e) => {
      ...
      document.onmousemove = function(e) {
        ...
        // 移动当前元素
        // 左侧限制移出800像素
        dragDom.style.left = `${between(l + styL, -800, document.body.clientWidth - 100)}px`
        dragDom.style.top = `${between(t + styT, 0, document.body.clientHeight - 100)}px`
        // 将此时的位置传出去
    ...
    // 双击头部全屏效果
    dialogHeaderEl.ondblclick_disable = (e) => {
